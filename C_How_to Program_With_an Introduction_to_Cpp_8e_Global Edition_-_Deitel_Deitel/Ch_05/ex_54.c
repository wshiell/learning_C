...
void askQuestion(int level, int type);

int main(void)
{
  int level, studentNumber = 0, type;
  ...
  if (studentNumber != -1) {
    ...
    puts("Enter problem category (1 for addition, 2 for subtraction, "
          "3 for multiplication, 4 for mixed type)");
    scanf("%d", &type);
  }

  while (studentNumber != -1){  

    askQuestion(level, type); 
