// This program demonstrates that as the number of squares a knight must visit increases, the time
// taken to complete a tour increases exponentially. For N squares, the program is run until a tour is
// completed successfully. Once the tour is completed, the number of attempts is recorded. The attempt
// is then made again, ten times in total for each value of N, and the average number of attempts in
// order to complete the tour successfully is recorded. The program then repeats this for N + 1 squares,
// and does so for all values up to N = 64, recording the average each time. This program only tries to
// achieve ten successful tours for each value of N, because for numbers greater than 10, the program
// will run far longer than my poor computer can handle (it's quite an old processor). I have also
// included a graph at the end, which you may notice is able to change in size depending on the maximum
// value of N. Feel free to change the values of TRIALS and MOVES (what I've referred to as N)
// to see what I mean.
...
#define MOVES 55 // Number of maxiumum moves
#define TRIALS 10 // Set the number of trial runs
