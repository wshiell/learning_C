...
void addQueen(int board[][8], int Q[][2], int count);
...
int main(void)
{
  ...
  while (attempts != -1) {
    ...
    int fail = 0;
    int board[8][8]; // Declare  array 'board' to zero
    int Q[8][2]; // Declare  array 'queen' to hold Qs position
    ...
    for (int count = 0; count < QUEENS; count++) {
      ...
      addQueen(board, Q, count); // Attempt to add queen # count to the board
